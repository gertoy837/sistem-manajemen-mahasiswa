...
<head>
    ...
    <title>Daftar Dosen - Sistem Manajemen Mahasiswa</title>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white shadow-sm border-b border-gray-200 px-8 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-500 text-sm">Kelola data dosen universitas</p>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto p-8">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    ...
                    <form action="${pageContext.request.contextPath}/dosen/search" method="get" class="flex gap-2">
                        <div class="relative">
                            <input type="text" name="keyword" value="${keyword}" placeholder="Cari NIP atau nama dosen..."
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Pendidikan</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Status</th>
                                    <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                <c:forEach var="dosen" items="${listDosen}" varStatus="loop">
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">${loop.index + 1}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="text-sm font-mono bg-gray-100 px-2 py-1 rounded">${dosen.nip}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="h-10 w-10 flex-shrink-0 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-white font-semibold">
                                                    ${dosen.namaDosen.substring(0, 1)}
                                                </div>
                                                <div class="ml-3">
                                                    <p class="text-sm font-medium text-gray-900">${dosen.namaDosen}</p>
                                                    <p class="text-xs text-gray-500">${dosen.jenisKelamin == 'L' ? 'Laki-laki' : 'Perempuan'}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">${dosen.namaJurusan}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">${dosen.email}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2.5 py-1 text-xs font-medium bg-purple-100 text-purple-700 rounded-full">${dosen.pendidikanTerakhir}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2.5 py-1 text-xs font-medium rounded-full ${dosen.status == 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
                                                ${dosen.status}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center space-x-2">
                                                <a href="${pageContext.request.contextPath}/dosen/edit?id=${dosen.idDosen}" 
                                                   class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a href="${pageContext.request.contextPath}/dosen/delete?id=${dosen.idDosen}" 
