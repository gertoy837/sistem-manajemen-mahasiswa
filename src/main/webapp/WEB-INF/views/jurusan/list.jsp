        ...
        <main class="flex-1 ml-64">
            <header class="bg-white/60 backdrop-blur-sm border-b border-slate-200/60 sticky top-0 z-40">
                <div class="flex items-center justify-between px-8 py-4">
                    <div>
                        ...
                        <p class="text-slate-500 text-sm">Kelola data jurusan/program studi</p>
                    </div>
                    <div class="flex items-center space-x-4">
                        <form action="${pageContext.request.contextPath}/jurusan/search" method="get" class="relative">
                            <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                            <input type="text" name="keyword" value="${keyword}" placeholder="Cari jurusan..." class="pl-11 pr-4 py-2.5 w-72 rounded-xl border border-slate-200 focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition-all text-sm">
                        </form>
                        <a href="${pageContext.request.contextPath}/jurusan/new" class="px-5 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-medium text-sm transition-colors">
            ...
            <div class="p-8">
                <c:if test="${not empty sessionScope.successMessage}">
                    <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl flex items-center" id="alertBox">
                        <i class="fas fa-check-circle mr-3"></i>${sessionScope.successMessage}
                        <c:remove var="successMessage" scope="session"/>
                    </div>
                </c:if>
                <c:if test="${not empty sessionScope.errorMessage}">
                    <div class="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl flex items-center" id="alertBox">
                        <i class="fas fa-exclamation-circle mr-3"></i>${sessionScope.errorMessage}
                ...
                <div class="bg-white rounded-2xl card-shadow overflow-hidden">
                        ...
                        <table class="w-full">
                            <thead class="bg-slate-50/80">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Nama Jurusan</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Fakultas</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Ketua Jurusan</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Akreditasi</th>
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <span class="font-medium text-slate-800">${jurusan.namaJurusan}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-slate-600">${jurusan.namaFakultas != null ? jurusan.namaFakultas : '-'}</td>
                                        <td class="px-6 py-4 text-sm text-slate-600">${jurusan.ketuaJurusan != null ? jurusan.ketuaJurusan : '-'}</td>
                                        <td class="px-6 py-4">
                                            <c:choose>
                                                <c:when test="${jurusan.akreditasi == 'A' || jurusan.akreditasi == 'Unggul'}">
                                                    <span class="px-2.5 py-1 text-xs font-medium bg-emerald-100 text-emerald-700 rounded-full">${jurusan.akreditasi}</span>
                                                </c:when>
                                                ...
                                                <c:otherwise>
                                                    <span class="px-2.5 py-1 text-xs font-medium bg-slate-100 text-slate-700 rounded-full">${jurusan.akreditasi != null ? jurusan.akreditasi : '-'}</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center justify-center space-x-1">
                                                <a href="${pageContext.request.contextPath}/jurusan/edit?id=${jurusan.idJurusan}" class="p-2 text-violet-600 hover:bg-violet-50 rounded-lg transition-colors">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a href="${pageContext.request.contextPath}/jurusan/delete?id=${jurusan.idJurusan}" onclick="return confirm('Hapus jurusan ini?')" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                    ...
                    <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100">
                        <p class="text-sm text-slate-500">Total: <span class="font-semibold text-slate-700">${fn:length(listJurusan)}</span> jurusan</p>
                    </div>
                </div>
            </div>
        </main>
