        ...
        <main class="flex-1 ml-64">
            <header class="bg-white/60 backdrop-blur-sm border-b border-slate-200/60 sticky top-0 z-40">
                <div class="flex items-center justify-between px-8 py-4">
                    <div>
                        <h2 class="text-xl font-bold text-slate-800">Daftar Mahasiswa</h2>
                        <p class="text-slate-500 text-sm">Kelola data mahasiswa</p>
                    </div>
                    <div class="flex items-center space-x-4">
                        <form action="${pageContext.request.contextPath}/mahasiswa" method="get" class="relative">
                            <input type="hidden" name="action" value="search">
                            <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                            <input type="text" name="keyword" value="${keyword}" placeholder="Cari mahasiswa..." class="pl-11 pr-4 py-2.5 w-72 rounded-xl border border-slate-200 focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition-all text-sm">
                        </form>
                        <a href="${pageContext.request.contextPath}/mahasiswa?action=add" class="px-5 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-medium text-sm transition-colors">
            ...
            <div class="p-8">
                <c:if test="${not empty message}">
                    <div class="mb-6 p-4 ${messageType == 'success' ? 'bg-emerald-50 border border-emerald-200 text-emerald-700' : 'bg-red-50 border border-red-200 text-red-700'} rounded-xl flex items-center" id="alertBox">
                        <i class="fas ${messageType == 'success' ? 'fa-check-circle' : 'fa-exclamation-circle'} mr-3"></i>
                        <span class="flex-1">${message}</span>
                        <button onclick="this.parentElement.remove()" class="text-current opacity-50 hover:opacity-100"><i class="fas fa-times"></i></button>
                    </div>
                </c:if>
                ...
                <div class="bg-white rounded-2xl card-shadow overflow-hidden">
                        ...
                        <table class="w-full">
                            <thead class="bg-slate-50/80">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">NIM</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Mahasiswa</th>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">IPK</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <c:forEach var="mhs" items="${listMahasiswa}" varStatus="loop">
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="px-2.5 py-1 text-xs font-mono bg-violet-100 text-violet-700 rounded-lg">${mhs.nim}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center space-x-3">
                                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center text-white font-semibold">
                                                    ${fn:toUpperCase(fn:substring(mhs.namaMahasiswa, 0, 1))}
                                                </div>
                                                <div>
                                                    <p class="font-medium text-slate-800">${mhs.namaMahasiswa}</p>
                                                    <p class="text-xs text-slate-500">${mhs.email}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-slate-600">${mhs.namaJurusan}</td>
                                        <td class="px-6 py-4">
                                            <span class="px-2.5 py-1 text-xs font-bold bg-indigo-100 text-indigo-700 rounded-full">Sem ${mhs.semesterAktif}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span class="text-sm font-semibold text-slate-700">${mhs.ipk}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <c:choose>
                                                <c:when test="${mhs.status == 'Aktif'}">
                                                    <span class="px-2.5 py-1 text-xs font-medium bg-emerald-100 text-emerald-700 rounded-full">${mhs.status}</span>
                                                </c:when>
                                                <c:when test="${mhs.status == 'Cuti'}">
                                                    <span class="px-2.5 py-1 text-xs font-medium bg-amber-100 text-amber-700 rounded-full">${mhs.status}</span>
                                                </c:when>
                                                <c:when test="${mhs.status == 'Lulus'}">
                                                    <span class="px-2.5 py-1 text-xs font-medium bg-blue-100 text-blue-700 rounded-full">${mhs.status}</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="px-2.5 py-1 text-xs font-medium bg-red-100 text-red-700 rounded-full">${mhs.status}</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center justify-center space-x-1">
                                                <a href="${pageContext.request.contextPath}/mahasiswa?action=edit&id=${mhs.idMahasiswa}" class="p-2 text-violet-600 hover:bg-violet-50 rounded-lg transition-colors"><i class="fas fa-edit"></i></a>
                                                <a href="${pageContext.request.contextPath}/mahasiswa?action=delete&id=${mhs.idMahasiswa}" onclick="return confirm('Hapus mahasiswa ${mhs.namaMahasiswa}?')" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><i class="fas fa-trash"></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                </c:forEach>
                    ...
                    <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100">
                        <p class="text-sm text-slate-500">Total: <span class="font-semibold text-slate-700">${fn:length(listMahasiswa)}</span> mahasiswa</p>
                    </div>
                </div>
            </div>
        </main>
    </div>
    <script>setTimeout(() => { const a = document.getElementById('alertBox'); if(a) a.remove(); }, 4000);</script>
</body>
</html>
